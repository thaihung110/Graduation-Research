    ...
    def serializer(message):
        def default_converter(o):
            if isinstance(o, datetime):
                return o.strftime("%Y-%m-%d %H:%M:%S")  # Format datetime as a string
            raise TypeError(f"Type {type(o)} not serializable")
        return json.dumps(message, default=default_converter).encode('utf-8')

    producer = KafkaProducer(
        bootstrap_servers= [f"{KAFKA_HOST}"],
        value_serializer=serializer,
        retries=5,
        batch_size=16384,  # Adjust batch size as necessary
    )
...
with DAG(
    ...
    (
        run_kafka_transformation_flight
    )